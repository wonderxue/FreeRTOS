...
void task1()
{
    ...
    char str[20];
    int x=xStreamBufferReceive(streamBuff,str,20,pdMS_TO_TICKS(10000));
...
void app_main()
{
    ...
    xStreamBufferSetTriggerLevel(streamBuff,8);
    xTaskCreate(task1,"task1",1200,NULL,3,&x1);
    xTaskCreate(task2,"task2",600,NULL,2,&x2);
    size_t xBytes=xStreamBufferSend(streamBuff,"xiao\n",6,pdMS_TO_TICKS(100));
    if(xBytes!=6)
    {
